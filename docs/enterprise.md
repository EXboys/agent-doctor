...
8. **派活在线**：`agent-doctor register --bootstrap-token <IT-token>` 写入 `evi_` 后运行 `agent-doctor connect`（WebSocket presence + inventory + **执行 job.assign**）。
...
| `POST /api/v1/engines/register` | `register`（IT bootstrap → 本机 `evi_`） |
...
# 5. 注册本机引擎（IT bootstrap → 写入 evi_）
agent-doctor register --bootstrap-token <IT-token>
# 可选：--runtime claude-code --engine-id my-laptop --rotate

# 6. 常驻在线（需 EVOTOWN_ENGINE_ID + evi_ 已写入 evotown.agent.env）
...
| `evotown-agent-setup.py register` | `agent-doctor register` |
| `evotown-agent-setup.py connector` | `agent-doctor connect`（在线 + 派活执行） |

仍保留在 Evotown 仓库的过渡能力：`watch`。`connect` 复用同一 `evi_` 做 WebSocket 鉴权与 job 结案。
