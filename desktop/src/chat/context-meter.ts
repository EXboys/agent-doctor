...
export function createContextMeterController(deps: ContextMeterDeps) {
  ...
  function positionContextPopover(): void {
    ...
    deps.contextPopoverEl.style.bottom = `${Math.round(window.innerHeight - rect.top + gap)}px`;
    deps.contextPopoverEl.style.top = "auto";
    deps.contextPopoverEl.style.zIndex = "120";
  }
  ...
  function updateContextMeter(): void {
    ...
    if (turns.length === 0 && !deps.promptEl.value.trim()) {
      closeContextPopover();
      deps.contextMeterEl.hidden = true;
      return;
    }
    deps.contextMeterEl.hidden = false;
    const pct = deps.contextUsagePercent(session, deps.promptEl.value);
    ...
    deps.contextLabelEl.textContent = t("chat.contextUsed", { pct: String(pct) });
    deps.contextMeterEl.classList.toggle("is-warn", pct >= 70 && pct < 90);
    deps.contextMeterEl.classList.toggle("is-full", pct >= 90);
    deps.contextMeterEl.title =
      pct >= 70 ? t("chat.contextNearFull") : t("chat.contextMeterTitle");
    if (deps.contextPopoverTitleEl) {
      deps.contextPopoverTitleEl.textContent = t("chat.contextPopoverTitle", {
        pct: String(pct),
      });
    }
    if (deps.contextPopoverBodyEl) {
      deps.contextPopoverBodyEl.textContent =
        pct >= 70
    ...
    if (deps.contextCompactEl) {
      ...
      deps.contextCompactEl.hidden = false;
    }
  }
